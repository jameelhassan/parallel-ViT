...
class TinyImageNetDataset(Dataset):
  def __init__(self, root_dir, mode='train', preload=False, load_transform=None,
    ...
    #                            dtype=np.float32)
    #   self.label_data = np.zeros((self.samples_num,), dtype=np.int16)
...
if __name__ == '__main__':
    ...
    train_dataset = TinyImageNetDataset(root_dir='./data', transform=train_transforms, preload=False)
    ...
    data1 = next(iter(val_dataset))
    print('You loaded the image')
    print(data1['image'].shape, data1['label'])